...
TRACE_EVENT(hmdfs_writepage_cb_enter,
	...
	TP_PROTO(struct hmdfs_peer *con, unsigned long long remote_ino,
		unsigned long page_index, int err, u64 ino_raw),

	TP_ARGS(con, remote_ino, page_index, err, ino_raw),
	...
	TP_fast_assign(
		...
		__entry->device_id = con->device_id;
		__entry->err = err;
		__entry->ino_raw = ino_raw;
	),

	TP_printk("hmdfs: src %s, writepage_cb start, return from remote device %llu, remote_ino=%llu, page_idx=%lu, err=%d, ino_raw=%llu",
		__entry->src, __entry->device_id,
		__entry->remote_ino, __entry->page_index, __entry->err,
		__entry->ino_raw)
);
...
TRACE_EVENT(hmdfs_writepage_cb_exit,
	...
	TP_printk("hmdfs: src %s, writepage_cb exit, return from remote device %llu, remote_ino=%llu, page_index=%lu, err=%d, ino_raw=%llu",
		__entry->src, __entry->device_id,
		__entry->remote_ino, __entry->page_index, __entry->err,
		__entry->ino_raw)
);
