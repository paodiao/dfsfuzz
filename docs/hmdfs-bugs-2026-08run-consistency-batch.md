...
- 冲突路径（`ofmyehonzz_33107412366.uas` / `woeokfsdmg_718021252744.odp`）在 **19 小时 campaign 内跨多次 crash 重启（snapshot 还原初始文件树）独立复现**：132327（08/31 13:23）→ 080706（09/01 08:07），同路径出现在 **9 个 crash bucket** 的比对输出中（17e6acbc/1f49816d/45c63e8e/49a94e7e/57d5b1cd/6512620c/86580a31/94d3c7ba/cb3ffa70）；
- 每次运行内，分歧进入全局树状态后持续参与比对，直至该次运行 crash 结束；snapshot 重启后文件树还原、分歧消失——跨 bucket 命中证明**竞争的可复现性**，而非单次运行的持续存在。
