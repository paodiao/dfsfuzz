...
func Default(target *prog.Target) (*ipc.Config, *ipc.ExecOpts, error) {
	...
	// Strip only a trailing empty element (from the trailing ';' convention
	// used by syz-manager/qemu when multiple executor commands are joined),
	// so that a plain value without a trailing ';' is kept intact.
	if len(executorCmds) > 0 && executorCmds[len(executorCmds)-1] == "" {
