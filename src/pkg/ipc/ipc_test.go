...
func TestExecute(t *testing.T) {
	...
	for _, flag := range flags {
		...
		for i := 0; i < 10; i++ {
			...
			output, infos, hanged, err, _, _, _ := env.Exec(opts, []*prog.Prog{p})
			if err != nil {
			...
			if len(infos) == 0 || len(infos[0].Calls) == 0 {
				t.Fatalf("no calls executed:\n%s", output)
			}
			if infos[0].Calls[0].Errno != 0 {
				t.Fatalf("simple call failed: %v\n%s", infos[0].Calls[0].Errno, output)
			}
			if len(output) != 0 {
