...
type ProgInfo struct {
	...
	// DAG feedback (schedule-level, hmdfs only): pair novelty bits, the
	// schedule hash bit and the raw pairs (DagSignal[k] corresponds to
	// DagPairs[k]). Filled by the fuzzer, never sent to the executor.
	DagSignal      []uint32
	DagScheduleBit uint32
...
func createMapShm(shmId int, size int) []byte {
	...
	inf, err := os.OpenFile(fn, os.O_RDWR, 0644)
	if err != nil {
		log.Fatalf("Open shared memory file %v failed: %v", fn, err)
	}
	mem, err := syscall.Mmap(int(inf.Fd()), 0, size,
		syscall.PROT_WRITE|syscall.PROT_READ, syscall.MAP_SHARED)
	if err != nil {
		log.Fatalf("Map shared memory failed: %v", err)
	}
	return mem
}
...
func MakeEnv(config *Config, pid int, shmId int) (*Env, error) {
	...
	for i := 1; i <= len(config.Executor); i++ {
		outmem := createMapShm(shmId+1+i, 16<<20)
		outmem[0] = 0
	...
	for idx, executorCmd := range config.Executor {
		...
		bins = append(bins,
			append(append(strings.Split(executorCmd, " "), "exec"), strconv.Itoa(isClient),
				...
				strconv.Itoa(config.MetadataDelayMs)))
...
func (env *Env) Exec(opts *ExecOpts, ps []*prog.Prog) (output []byte, infos []*ProgInfo, hanged bool,
	err0 error, fsMds []map[string]prog.FileMetadata, testdirIno uint64, hmdfsTraceEvents []prog.HmdfsTraceEvent) {
	...
	if len(ps) > 64 {
		err0 = fmt.Errorf("too many programs: %v (max 64)", len(ps))
		return
	}
	for idx, p := range ps {
	...
	*/
	...
	output, hanged, err0 = env.cmd.exec(opts, progData, progSizes, progOffsets, 0, 0)
	if err0 != nil {
	...
	infos, err0, executor_idx, _, fsMds, testdirIno, hmdfsTraceEvents = env.parseOutputs(ps)
...
func (env *Env) parseOutputs(ps []*prog.Prog) ([]*ProgInfo, error, int, *prog.Prog,
	[]map[string]prog.FileMetadata, uint64, []prog.HmdfsTraceEvent) {
	...
	for i := 0; i < progsLen; i++ {
		ret := <-retChan
		if ret.err != nil {
			return nil, ret.err, ret.idx, nil, nil, 0, nil
		}
		...
		if ret.hmdfsTraceEvents != nil {
			hmdfsTraceEvents = append(hmdfsTraceEvents, ret.hmdfsTraceEvents...)
		}
	}
	...
	return append(srvInfos, clientInfos...), nil, 0, symc3Prog, fsMds, testdirIno, hmdfsTraceEvents
}
...
func (env *Env) parseFsMd(outp *[]byte) (map[string]prog.FileMetadata, error) {
	...
	for i := 0; i < stat_cnt; i++ {
		...
		if uint32(len(out)) < filepathSize {
			return nil, fmt.Errorf("bad filepath size %v/%v", filepathSize, len(out))
		}
		...
		//xattr
		if uint32(len(out)) < xattrSize {
			return nil, fmt.Errorf("bad xattr size %v/%v", xattrSize, len(out))
		}
		xattrs := ""
		if xattrSize != 0 {
		...
		if uint32(len(out)) < statSize {
			return nil, fmt.Errorf("bad stat size %v/%v", statSize, len(out))
		}
...
func parseHmdfsTraceEvents(data *[]byte) []prog.HmdfsTraceEvent {
	...
	for i := uint32(0); i < count; i++ {
		ts, ok1 := readUint64(&out)
		fid, ok2 := readUint32(&out)
		ret, ok3 := readUint32(&out)
		ino, ok4 := readUint64(&out)
		if !ok1 || !ok2 || !ok3 || !ok4 {
			break
		}
		events[i] = prog.HmdfsTraceEvent{
			Timestamp: ts,
			FuncID:    fid,
			Ret:       int32(ret),
			Ino:       ino,
		...
		for j := 0; j < 6; j++ {
			v, ok5 := readUint64(&out)
			if !ok5 {
				break
			}
			events[i].Args[j] = v
		}
		off, ok6 := readUint64(&out)
		if !ok6 {
			break
		}
		events[i].Off = off
...
func (env *Env) parseClientOutput(p *prog.Prog, idx int, retChan chan parseRet) {
	...
	if env.config.EnableCsan && idx >= env.config.ServNum {
		...
		// Consume the stream, discard the result: fsMds slots for servers /
		// non-csan clients stay nil to keep the consumer semantics unchanged.
		if _, err = env.parseFsMd(&out); err != nil {
			retChan <- parseRet{info: info, fsMd: nil, err: err, idx: idx}
			return
		}
	}
...
type FileOpResultHeader struct {
	Magic    uint32
	Count    uint32
	DataSize uint32
	//Reserved uint32
	//TODO: 这两个Reserved不匹配，两边的代码都是过了前面的字节后就拿实际内容了
}
	...
	*/
	...
	go func() {
		...
		for i := startIdx; i < stopIdx; i++ {
			...
			for atomic.LoadUint32((*uint32)(unsafe.Pointer(&c.outmems[i][0])))&0xff != 1 {
			}
...
func (c *command) exec(opts *ExecOpts, progData []byte, progSizes [64]uint64, progOffsets [64]uint64, fileOpsOffset uint64, fileOpsSize uint64) (output []byte, hanged bool, err0 error) {
	...
	req := &executeReq{
		...
		fileOpsOffset:    fileOpsOffset,
		fileOpsSize:      fileOpsSize,
	}
	...
	if c.config.UseShmem {
			...
			for i := len(c.outmems) - 1; i >= 0; i-- {
				...
				go func(idx int, wg *sync.WaitGroup) {
					...
					for msg == 0 {
						...
						default:
							//if c.outmems[i][0] != 0 {
							msg = atomic.LoadUint32((*uint32)(unsafe.Pointer(&c.outmems[idx][0]))) & 0xff
							if msg != 0 {
		...
		case <-time.After(c.config.Timeouts.Program*100 + 30*time.Second):
			stop <- true
