...
import (
	...
	"monarch/pkg/osutil"
	"monarch/prog"
	_ "monarch/sys" // most mgrconfig users want targets too
	"monarch/sys/targets"
)
...
type Derived struct {
	...
	ExecprogBin        string
	ExecutorBin        string
	HostFuzzerCheckBin string
	DFSConfigBins	   string
...
func Complete(cfg *Config) error {
	...
	if cfg.Image != "" {
		...
		for _, image := range strings.Split(cfg.Image, ";") {
			if !osutil.IsExist(image) {
				return fmt.Errorf("bad config param image: can't find %v", image)
			}
			tmp += osutil.Abs(image) + ";"
		}
		cfg.Image = tmp
	}
	...
	if cfg.ServNum > cfg.FuzzingVMs {
		return fmt.Errorf("server_num %v exceeds fuzzing_vms %v", cfg.ServNum, cfg.FuzzingVMs)
	}
...
func (cfg *Config) completeBinaries() error {
	...
	cfg.HostFuzzerCheckBin = targetBin("syz-features-check-ssh", cfg.TargetArch)
	cfg.DFSConfigBins = filepath.Join(cfg.Syzkaller, "dfs-config", cfg.DFSName)
