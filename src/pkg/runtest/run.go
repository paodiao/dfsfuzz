...
func parseProg(target *prog.Target, dir, filename string) (*prog.Prog, map[string]bool, *ipc.ProgInfo, error) {
	...
	//
	p, err := target.Deserialize(datas, prog.Strict)
	if err != nil {
	...
	//TODO
	info := &ipc.ProgInfo{Calls: make([]ipc.CallInfo, len(p[0].Calls))}
	//
	for i, call := range p[0].Calls {
...
func RunTest(req *RunRequest, executor string) {
	...
	req.Cfg.Executor = append(executors, executor)
	var env *ipc.Env
	defer func() {
		if env != nil {
			env.Close()
	...
	for run := 0; run < req.Repeat; run++ {
		...
		var ps []*prog.Prog
		ps = append(ps, req.P)
		output, info, hanged, err, _, _, _, _, _ := env.Exec(req.Opts, ps)
		//tao end
