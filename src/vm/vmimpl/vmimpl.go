...
type Pool interface {
	...
	Create(workdir string, index int) (Instance, error, io.ReadCloser)
}
...
type Instance interface {
	...
	Run(timeout time.Duration, stop <-chan bool, command string, qemuRPipes []io.ReadCloser) (outc <-chan []byte, errc <-chan error, err error)

	//tao added: generate executors' starting cmd
	GenExecutorCmd(command string) (string, string, error)
	GetSSHArgs(bin string) []string
	//tao end
