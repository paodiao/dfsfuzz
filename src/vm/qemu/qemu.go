...
type Config struct {
	...
	InitialDir     string `json:"initial_dir"`
...
func (inst *instance) boot(index int) error {
	...
	// 16MB to match executor kMaxOutput (metadata dump of the merge_view tree)
	shmId := inst.cfg.InitShmId + index + 2
	objArg := fmt.Sprintf("memory-backend-file,size=16M,share=on,mem-path=/dev/shm/shm%d,id=shm%d", shmId, shmId)
	...
	*/
	...
	if inst.cfg.Kernel != "" {
		...
		// hmdfs uses the full ip= format with an explicit eth0 (relying on
		// net.ifnames=0 in the vm cmdline) and DHCP disabled, matching the
		// hand-written qemu command used to set up a dfs instance.
		ipArg := "ip=" + inst.ip
		if inst.cfg.DFSName == "hmdfs" {
			ipArg = "ip=" + inst.ip + ":::::eth0:off"
		}
		cmdline = append(cmdline, inst.cfg.Cmdline, ipArg, "quiet", "nokaslr") //Guest ip start from 192.160.0.2
		args = append(args,
...
func (inst *instance) Run(timeout time.Duration, stop <-chan bool, command string,
	...
	if bin := filepath.Base(args[0]); inst.target.HostFuzzer &&
		...
		tmpArgs = append(tmpArgs, fmt.Sprintf("-InitialDir=%s", inst.cfg.InitialDir))
