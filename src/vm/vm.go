...
func Create(cfg *mgrconfig.Config, debug bool) (*Pool, error) {
	...
	env := &vmimpl.Env{
		...
		ServNum:        cfg.ServNum,
		InitIp:         cfg.InitIp,
		FuzzingVMs:     cfg.FuzzingVMs,
		InitShmId:      cfg.InitShmId,
		DfsSetupParams: cfg.DfsSetupParams,
		InitialDir:     cfg.InitialDir,
		KernelSrv:      cfg.KernelSrv,
		KernelClient:   cfg.KernelClient,
		NetFailure:     cfg.NetFailure,
		NodeCrash:      cfg.NodeCrash,
		LFSBased:       cfg.LFSBased,
		EnableCsan:     cfg.EnableCsan,
		EnableC2san:    cfg.EnableC2san,
		EnableSrvFb:    cfg.EnableSrvFb,
		EnableEval:     cfg.EnableEval,
		EnableClientFb: cfg.EnableClientFb,
		Syzkaller:      cfg.Syzkaller,
	}
...
func (pool *Pool) Create(index int) (*Instance, error) {
	...
	impl, err, rpipe := pool.impl.Create(workdir, index)
	if err != nil {
...
func (inst *Instance) GetSSHArgs(bin string) []string {
	return inst.impl.GetSSHArgs(bin)
}

func (inst *Instance) GenExecutorCmd(command string) (string, string, error) {
	return inst.impl.GenExecutorCmd(command)
}

func (inst *Instance) Run(timeout time.Duration, stop <-chan bool, command string,
	qemuRPipes []io.ReadCloser) (outc <-chan []byte, errc <-chan error, err error) {
	return inst.impl.Run(timeout, stop, command, qemuRPipes)
}
