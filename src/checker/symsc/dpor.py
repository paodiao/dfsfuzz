    ...
    if c.runtime_state[next_op]["Stime"] >= c.runtime_state[s[1]]["Etime"] or \
       c.runtime_state[s[1]]["Stime"] >= c.runtime_state[next_op]["Etime"]:
        ...
        # File tree related operations
            ...
            elif call_seq in ["getdents, open", "getdents, close", "getdents64, open", "getdents64, close"]:
                return True if fd1 == fd2 else False
            elif call_seq in ["getdents, dup", "getdents, dup2", "getdents64, dup", "getdents64, dup2"]:
                return True if fd1 == [fd2[0]] else False
            elif call_seq in ["getdents, link", "getdents, symlink", "getdents64, link", "getdents64, symlink"]:
                return True if is_parent(path1, [path2[1]]) else False
            elif call_seq in ["getdents, unlink", "getdents64, unlink"]:
                return True if is_parent(path1, path2) else False 
            elif call_seq in ["getdents, chmod", "getdents, fchmod", "getdents64, chmod", "getdents64, fchmod"]:
