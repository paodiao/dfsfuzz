...
/*
#if SYZ_EXECUTOR || __NR_syz_net_down_sleep_up
#include <stdlib.h>
static long syz_net_down_sleep_up(volatile long usec) {
  //system("ip link set eth0 down");
  //usleep(usec); // microseconds
  //system("ip link set eth0 up");
  system(
      "iptables -A INPUT -s 192.168.0.1 -j ACCEPT; iptables -A OUTPUT -d
192.168.0.1 -j ACCEPT; \ iptables -P INPUT DROP; iptables -P OUTPUT DROP");
  usleep(usec);
  system("iptables -P INPUT ACCEPT; iptables -P OUTPUT ACCEPT");
...
static long syz_failure_net_down(volatile char *cmd) {
  ...
  // system("iptables -A INPUT -s 192.168.0.1 -j ACCEPT; iptables -A OUTPUT -d
  // 192.168.0.1 -j ACCEPT; \ iptables -P INPUT DROP; iptables -P OUTPUT DROP");
  return 0;
}
...
static long syz_net_delay_add(volatile char *cmd) {
  debug_cmd((const char *)cmd);
  return 0;
}

static long syz_net_delay_del() {
  debug_cmd("tc qdisc del dev eth0 root 2>/dev/null");
  return 0;
}
#endif

/*
#if SYZ_EXECUTOR || __NR_syz_usleep
#include <unistd.h>

static long syz_usleep(volatile long sleep_usec) {
  usleep(sleep_usec);
  return 0;
}
#endif
...
extern volatile struct executeControl *execCtl;
void sync_lock();
void sync_unlock();
#if SYZ_EXECUTOR || __NR_syz_failure_sync || __NR_syz_failure_send ||          \
    __NR_syz_failure_recv || __NR_syz_failure_barrier
extern uint64 server_num, vm_count;
#define SUPPORT_VMS 64
struct executeControl {
  volatile unsigned char hasTestcase[SUPPORT_VMS];
  volatile unsigned char coverEnabled[SUPPORT_VMS];
  volatile unsigned char executionsFinish[SUPPORT_VMS];
  volatile unsigned char tmpDirEstablished;
  volatile uint64_t synchBit;
  volatile unsigned char barrierCount[64];
  volatile atomic<unsigned char> lockByte2; // for concurrent call lock
  volatile atomic<unsigned char> lockByte3; // for sync lock
  volatile uint64_t srvSetupBit;
};
//修改：MAXCLT还需要扩充吗？
#define MAXCLT 3
#define SHIFTBIT (MAXCLT + 1)

static long syz_failure_barrier(unsigned char idx, unsigned char nodecount) {
  if(nodecount > vm_count) {
    fail("The number of nodes exceeds the vm count at syz_failure_barrier");
  }

  sync_lock();
  execCtl->barrierCount[idx]++;
  sync_unlock();
  fprintf(stderr, "executor %lld waits for recv msg in barrier %u\n", executor_index, idx);
  // recv msg
  while (execCtl->barrierCount[idx] < nodecount) {
        usleep(10);
    }
  return 0;
}

static long syz_failure_sync(unsigned char idx, unsigned char clt) {
  if (clt - server_num >= MAXCLT) {
    fail("The number of clients exceeds the maximum client bits at "
         "syz_failure_start");
  }
  /*layout
  | clt1send | clt2send | ... | recv |
  */
  // send msg
  sync_lock();
  execCtl->synchBit |= ((uint64_t)1 << (idx * SHIFTBIT)) << (clt - server_num);
  sync_unlock();
  fprintf(stderr, "executor %lld waits for recv msg\n", executor_index);
  // recv msg
  uint64_t mask = ((uint64_t)1 << (idx * SHIFTBIT) << MAXCLT);
  while (!(execCtl->synchBit & mask)) {
    usleep(10);
  }
  return 0;
}

static long syz_failure_send(unsigned char idx) {
  sync_lock();
  execCtl->synchBit |= ((uint64_t)1 << (idx * SHIFTBIT) << MAXCLT);
  sync_unlock();
  return 0;
}

static long syz_failure_recv(unsigned char idx) {
  for (uint64_t i = 0; i < (vm_count - server_num); i++) {
    if (executor_index >= server_num && i == executor_index - server_num) {
      continue;
    }
    uint64_t mask = ((uint64_t)1 << (idx * SHIFTBIT)) << i;
    while (!(execCtl->synchBit & mask)) {
      usleep(10);
    }
  }
...
struct outputControl {
  char executionFinished;
  int cntTmp;
  uint32 *outputPosTmp;
};

extern char *dfs_setup_params, *init_ip, *dfs_name;
char test_dir[100];
#if SYZ_EXECUTOR || __NR_syz_failure_crash_client ||                           \
    __NR_syz_failure_crash_server
void exec_unlock(int call_index);
void exec_lock();
extern volatile struct outputControl *output_ctl_pos;
static long syz_failure_crash_client(int willCrash) {
  // release lock before crash
  exec_unlock(-1);

  sync_lock();
  execCtl->synchBit |= uint64_t(1) << (executor_index - server_num);
  sync_unlock();

  if (willCrash) {
    // wait all clients finish
    for (uint64_t i = 0; i < (vm_count - server_num); i++) {
      uint64_t mask = ((uint64_t)1) << i;
      while (!(execCtl->synchBit & mask)) {
      }
    }
    // TODO: if crash here, please add exec_unlock() at the beginning of this
    // function output_ctl_pos->executionFinished = 2;
  ...
  // list the test_dir until it succeeds.
  if (!strcmp(dfs_name, "glusterfs")) {
    snprintf(test_dir, 100, "/root/%s-client/", dfs_name);
  } else if(!strcmp(dfs_name, "hmdfs")) {
    snprintf(test_dir, 100, "/mnt/hmdfs/100/non_account");
  } else {
    snprintf(test_dir, 100, "/root/%s-client/dfs-0-%lld", dfs_name,
             execution_index);
  }
  int times = 0, LIMIT = 10000;
  struct stat test_dir_stat;
  while (stat(test_dir, &test_dir_stat) < 0 && (times < LIMIT)) {
    usleep(1000);
    times++;
  }
  if (times >= LIMIT) {
    fail("File system is not accessable from clients after the crash and "
         "recovery");
  }
  // char fname[100];
  // snprintf(fname, 100, "/root/%s-node-up.sh %s %s %lld", dfs_name, init_ip,
  // dfs_setup_params, executor_index); system(fname); Workaround for the
  // exec_unlock in execute_syscall
  exec_lock();
  return 0;
}
static long syz_failure_crash_server() {
  // release lock before crash
  exec_unlock(-1);
  sync_lock();
  ...
  // debug_cmd("sync -f /root/glusterfs-server");
  output_ctl_pos->executionFinished = 2;
#if MDEBUG
  struct timeval tv;
  gettimeofday(&tv, NULL);
  fprintf(stderr, "node_crash testing: executor %lld crashes at %ld:%ld\n",
...
/*
#if SYZ_EXECUTOR || __NR_syz_reboot
#include <sys/reboot.h>
#include <unistd.h>

extern volatile struct outputControl *output_ctl_pos;
extern int global_call_index;
static long syz_reboot() {
  system("systemctl stop ssh");
  output_ctl_pos->executionFinished =
      global_call_index +
      2; // because 0 and 1 has been assigned for whether there is output.

  // reboot(RB_AUTOBOOT);
  // KCOV_DISABLE
  // if(ioctl(usp_covers[0].fd, KCOV_DISABLE, 0) == -1)
  //  fail("remote server cover disable before reboot failed\n");
  // fprintf(stderr, "usp_covers[0].fd %d\n", usp_covers[0].fd);
  //shm_need_reboot[0] = 1;
  doexit(0);
  // system("reboot");
...
#if SYZ_EXECUTOR || __NR_syz_failure_up || __NR_syz_failure_down
extern uint32 kernel_server;
extern int global_call_index;
#define CALLOFFSET 3
// Regarding to node crashes, now we only fail server nodes.
// For kernel servers, we use sysrq.
// For usrspace servers, we use process killing.
static long syz_failure_down() {

  if (kernel_server) {

    // label which server crashed. All crashed nodes have to recover after its
    // procedding nodes's recovery.
    sync_lock();
    execCtl->srvSetupBit |= ((uint64_t)1) << executor_index;
    sync_unlock();

    // tell fuzzer re-execute the executor
    // we are usng global variable global_call_index, which is assigned to the
    // call and then plus one. so here we need to minus 1.
    output_ctl_pos->executionFinished = CALLOFFSET + global_call_index - 1;
    debug_cmd("echo b > /proc/sysrq-trigger");

  } else {
    char cmd[100];
    snprintf(cmd, 100, "/root/%s-node-down.sh", dfs_name);
    debug_cmd(cmd);
    // reinitialize the pool of coverage memory
    reinit_cov_shms();
  }
  return 0;
}
...
#if SYZ_EXECUTOR || SYZ_REPEAT || SYZ_NET_INJECTION || SYZ_FAULT ||            \
    SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID || SYZ_SANDBOX_NAMESPACE ||         \
    SYZ_SANDBOX_ANDROID || SYZ_FAULT || SYZ_LEAK || SYZ_BINFMT_MISC ||         \
    SYZ_SYSCTL ||                                                              \
...
static bool write_file(const char *file, const char *what, ...) {
  ...
  return true;
    ...
    SYZ_WIFI || SYZ_802154 || __NR_syz_genetlink_get_family_id ||              \
...
static int netlink_send_ext(struct nlmsg *nlmsg, int sock, uint16 reply_type,
  ...
  if (n != (ssize_t)hdr->nlmsg_len) {
    if (dofail)
      failmsg("netlink_send_ext: short netlink write", "wrote=%zd, want=%d", n,
              hdr->nlmsg_len);
  ...
  if (hdr->nlmsg_type != NLMSG_ERROR) {
    errno = EINVAL;
    if (dofail)
      failmsg("netlink_send_ext: bad netlink ack type", "type=%d",
              hdr->nlmsg_type);
...
#if SYZ_EXECUTOR || SYZ_NET_DEVICES || SYZ_NET_INJECTION || SYZ_DEVLINK_PCI || \
    SYZ_WIFI || SYZ_802154 || __NR_syz_80211_join_ibss ||                      \
    __NR_syz_80211_inject_frame
...
static void netlink_add_vlan(struct nlmsg *nlmsg, int sock, const char *name,
                             const char *link, uint16 id, uint16 proto) {
...
static void netlink_add_geneve(struct nlmsg *nlmsg, int sock, const char *name,
                               uint32 vni, struct in_addr *addr4,
...
static void netlink_add_ipvlan(struct nlmsg *nlmsg, int sock, const char *name,
                               const char *link, uint16 mode, uint16 flags) {
...
static void netlink_device_change(struct nlmsg *nlmsg, int sock,
  ...
  if (err < 0) {
    debug("netlink: device %s up master %s: %s\n", name,
          master ? master : "NULL", strerror(errno));
  }
}
#endif
...
static void initialize_tun(void) {
  ...
  // fd numbers stable regardless of whether tun is opened or not (also see
  // kMaxFd).
  ...
  // Note: SYZ_ENABLE_NAPI_FRAGS is never enabled. This is code is only for
  // reference in case we figure out how IFF_NAPI_FRAGS works. With
  // IFF_NAPI_FRAGS packets don't reach destinations and bail out in
  // udp_gro_receive (see #1594). Also IFF_NAPI_FRAGS does not work with
  // sandbox_namespace (see comment there).
#if ENABLE_NAPI_FRAGS
  ...
  // Disable IPv6 DAD, otherwise the address remains unusable until DAD
  // completes. Don't panic because this is an optional config.
  char sysctl[64];
...
static void netlink_devlink_netns_move(const char *bus_name,
  ...
  if (err < 0) {
    debug("netlink: failed to move devlink instance %s/%s into network "
          "namespace: %s\n",
...
#if SYZ_EXECUTOR || SYZ_WIFI || __NR_syz_80211_inject_frame ||                 \
    __NR_syz_80211_join_ibss
...
static int nl80211_set_interface(struct nlmsg *nlmsg, int sock,
                                 int nl80211_family, uint32 ifindex,
                                 uint32 iftype) {
  struct genlmsghdr genlhdr;
...
static int nl80211_join_ibss(struct nlmsg *nlmsg, int sock, int nl80211_family,
                             uint32 ifindex, struct join_ibss_props *props) {
  struct genlmsghdr genlhdr;
...
static int nl80211_setup_ibss_interface(struct nlmsg *nlmsg, int sock,
  ...
  int ifindex = if_nametoindex(interface);
  if (ifindex == 0) {
    debug("nl80211_setup_ibss_interface: if_nametoindex failed for %.32s, ret "
          "0\n",
          interface);
    return -1;
  }
  ...
  if (ret < 0) {
    debug("nl80211_setup_ibss_interface: nl80211_set_interface failed for "
          "%.32s, ret %d\n",
          interface, ret);
    return -1;
  }
  ...
  if (ret < 0) {
    debug("nl80211_setup_ibss_interface: set_interface_state failed for %.32s, "
          "ret %d\n",
          interface, ret);
    return -1;
  }
  ...
  if (ret < 0) {
    debug("nl80211_setup_ibss_interface: nl80211_join_ibss failed for %.32s, "
          "ret %d\n",
          interface, ret);
    return -1;
  }
...
static void initialize_wifi_devices(void) {
  ...
  // An IBSS network is created here in order to put these devices in an
  // operable state right from the beginning. It has the following positive
  // effects.
  // 1. Frame injection becomes possible from the very start.
  // 2. A number of nl80211 commands expect their target wireless interface to
  // be in an operable state.
  // 3. Simplification of reproducer generation - in many cases the reproducer
  ...
  for (int device_id = 0; device_id < WIFI_INITIAL_DEVICE_COUNT; device_id++) {
    ...
    if (ret < 0)
      failmsg("initialize_wifi_devices: failed to create device", "device=%d",
              device_id);

    // For each device, unless HWSIM_ATTR_NO_VIF is passed, a network interface
    // is created automatically. Such interfaces are named "wlan0", "wlan1" and
    // so on.
    ...
    if (nl80211_setup_ibss_interface(&nlmsg, sock, nl80211_family_id, interface,
                                     &ibss_props) < 0)
      failmsg("initialize_wifi_devices: failed set up IBSS network",
              "device=%d", device_id);
  }
...
// We test in a separate namespace, which does not have any network devices
// initially (even lo). Create/up as many as we can.
static void initialize_netdevices(void) {
  ...
  // Also init namespace contains the following devices (which presumably can't
  // be created in non-init namespace), can we use them somehow?
  // - ifb0/1
  // - teql0
  // - eql
  char netdevsim[16];
  ...
  struct {
    ...
    const char *dev;
      ...
      // Note: ip6erspan device can't be added if ip6gretap exists in the same
      // namespace.
      ...
      {"caif", "caif0"},
      {"batadv", "batadv0"},
      // Note: this adds vxcan0/vxcan1 pair, similar to veth (creating vxcan0
      // would fail).
      {"vxcan", "vxcan1"},
      // Note: netdevsim devices can't have the same name even in different
      // namespaces.
      {"netdevsim", netdevsim},
  ...
  netdevsim_add((int)procid, 4); // Number of port is in sync with value in
                                 // sys/linux/socket_netlink_generic_devlink.txt

  netlink_wireguard_setup();

  for (i = 0; i < sizeof(devices) / (sizeof(devices[0])); i++) {
    // Assign some unique address to devices. Some devices won't up without
    // this. Shift addresses by 10 because 0 subnet address can mean special
    // things.
    char addr[32];
...
#if SYZ_EXECUTOR ||                                                            \
    SYZ_NET_INJECTION && (__NR_syz_extract_tcp_res || SYZ_REPEAT)
#include <errno.h>
...
static long syz_emit_ethernet(volatile long a0, volatile long a1,
                              volatile long a2) {
  // syz_emit_ethernet(len len[packet], packet ptr[in, eth_packet], frags
  // ptr[in, vnet_fragmentation, opt]) vnet_fragmentation { 	full
  // int32[0:1] 	count	int32[1:4] 	frags	array[int32[0:4096], 4]
  // }
  if (tunfd < 0)
    return (uintptr_t)-1;
  ...
  return write(tunfd, data, length);
    ...
    __NR_syz_io_uring_setup

#define SIZEOF_IO_URING_SQE 64
#define SIZEOF_IO_URING_CQE 16

// Once a io_uring is set up by calling io_uring_setup, the offsets to the
// member fields to be used on the mmap'ed area are set in structs
// io_sqring_offsets and io_cqring_offsets. Except io_sqring_offsets.array, the
// offsets are static while all depend on how struct io_rings is organized in
// code. The offsets can be marked as resources in syzkaller descriptions but
// this makes it difficult to generate correct programs by the fuzzer. Thus, the
// offsets are hard-coded here (and in the descriptions), and array offset is
// later computed once the number of entries is available. Another way to obtain
// the offsets is to setup another io_uring here and use what it returns. It is
// slower but might be more maintainable.
...
static long syz_io_uring_complete(volatile long a0) {
  ...
  // Advance the head. Head is a free-flowing integer and relies on natural
  // wrapping. Ensure that the kernel will never see a head update without the
  // preceeding CQE stores being done.
  ...
  // with a unique range of sqe.user_data (0x12345 and 0x23456) to identify the
  // operations which produces an fd instance. Check cqe.user_data, which should
  // be the same as sqe.user_data for that operation. If it falls in that unique
  // range, return cqe.res as fd. Otherwise, just return an invalid fd.
...
static long syz_io_uring_setup(volatile long a0, volatile long a1,
  ...
  // ptr[inout, io_uring_params], addr_ring vma, addr_sqes vma, ring_ptr
  // ptr[out, ring_ptr], sqes_ptr ptr[out, sqes_ptr]) fd_io_uring C:
  // syz_io_uring_setup(uint32 entries, struct io_uring_params* params, void*
  // mmap_addr_ring, void* mmap_addr_sqes, void** ring_ptr_out, void**
  // sqes_ptr_out) // returns uint32 fd_io_uring
  ...
  uint32 cq_ring_sz = setup_params->cq_off.cqes +
...
static long syz_io_uring_submit(volatile long a0, volatile long a1,
  ...
  // sqe ptr[in, io_uring_sqe],   sqes_index int32) C: syz_io_uring_submit(char*
  // ring_ptr,       io_uring_sqe* sqes_ptr,    io_uring_sqe* sqe, uint32
  // sqes_index)
...
static long syz_usbip_server_init(volatile long a0) {
  ...
  // Each port number corresponds to a particular vhci_hcd (USB/IP Virtual Host
  // Controller) and it is used by either an usb2 device or usb3 device. There
  // are 16 ports available in each vhci_hcd. (VHCI_PORTS = 16 in our case.)
  // When they are occupied, the following vhci_hcd's ports are used. First 16
  // ports correspond to vhci_hcd0, next 16 ports correspond to vhci_hcd1 etc.
  // In a vhci_hcd, first 8 ports are used by usb2 devices and last 8 are used
  // by usb3 devices.
  int port_num =
  ...
  // When fuzzing with syzkaller we don't have an actual server or an actual
  // device, so use 0 for devid.
  char buffer[100];
...
static char *read_btf_vmlinux() {
  ...
  for (;;) {
    ssize_t ret =
        read(fd, buf + bytes_read, VMLINUX_MAX_SUPPORT_SIZE - bytes_read);
...
static long syz_btf_id_by_name(volatile long a0) {
  ...
  while (bytes_parsed < btf_header->type_len) {
    struct btf_type *btf_type =
        (struct btf_type *)(btf_type_sec + bytes_parsed);
...
static long syz_memcpy_off(volatile long a0, volatile long a1, volatile long a2,
  ...
  // C:       syz_memcpy_off(void* dest, uint32 dest_off, void* src, uint32
  // src_off, size_t n)
...
static long syz_extract_tcp_res(volatile long a0, volatile long a1,
                                volatile long a2) {
  // syz_extract_tcp_res(res ptr[out, tcp_resources], seq_inc int32, ack_inc
  // int32)

  if (tunfd < 0)
    return (uintptr_t)-1;

  // We just need this to be large enough to hold headers that we parse
  // (ethernet/ip/tcp). Rest of the packet (if any) will be silently truncated
  // which is fine.
  char data[1000];
...
static long syz_open_dev(volatile long a0, volatile long a1, volatile long a2) {
  if (a0 == 0xc || a0 == 0xb) {
    ...
    char buf[128];
    sprintf(buf, "/dev/%s/%d:%d", a0 == 0xc ? "char" : "block", (uint8)a1,
            (uint8)a2);
    ...
    while ((hash = strchr(buf, '#'))) {
      *hash =
          '0' + (char)(a1 % 10); // 10 devices should be enough for everyone.
      a1 /= 10;
    }
...
#if SYZ_EXECUTOR || SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID ||                  \
    SYZ_SANDBOX_NAMESPACE || SYZ_SANDBOX_ANDROID
#include <fcntl.h>
#include <sched.h>
#include <sys/stat.h>
#include <sys/types.h>
...
static void hci_send_event_packet(int fd, uint8 evt, void *data,
                                  size_t data_len) {
  struct iovec iv[3];
...
static void hci_send_event_cmd_complete(int fd, uint16 opcode, void *data,
                                        size_t data_len) {
  struct iovec iv[4];
...
static void initialize_vhci() {
  ...
  // fd numbers stable regardless of whether vhci is opened or not (also see
  // kMaxFd).
...
static long syz_genetlink_get_family_id(volatile long name,
  ...
  // We can't trust the socket passed by the fuzzer, it may be not a netlink at
  // all.
  bool dofail = false;
  int fd = sock_arg;
  if (fd < 0) {
...
static int setup_loop_device(long unsigned size, long unsigned nsegs,
                             struct fs_image_segment *segs,
                             const char *loopname, int *memfd_p,
                             int *loopfd_p) {
  int err = 0, loopfd = -1;
...
// syz_read_part_table(size intptr, nsegs len[segments], segments ptr[in,
// array[fs_image_segment]])
...
// syz_mount_image(fs ptr[in, string[disk_filesystems]], dir ptr[in, filename],
// size intptr, nsegs len[segments], segments ptr[in, array[fs_image_segment]],
// flags flags[mount_flags], opts ptr[in, fs_options[vfat_options]]) fd_dir
// fs_image_segment {
...
static long syz_mount_image(volatile long fsarg, volatile long dir,
  ...
  debug("syz_mount_image: size=%llu segs=%llu loop='%s' dir='%s' fs='%s' "
        "flags=%llu opts='%s'\n",
...
// checkpoint/reset_net_namespace partially resets net namespace to initial
// state after each test. Currently it resets only ipv4 netfilter state.
...
// Linux headers do not compile for C++, so we have to define the structs
// manualy.
...
static void checkpoint_iptables(struct ipt_table_desc *tables, int num_tables,
  ...
  if (fd == -1) {
    ...
    failmsg("iptable checkpoint: socket(SOCK_STREAM, IPPROTO_TCP) failed",
            "family=%d", family);
  }
  for (int i = 0; i < num_tables; i++) {
    ...
    if (table->info.size > sizeof(table->replace.entrytable))
      failmsg("iptable checkpoint: table size is too large",
              "table=%s, family=%d, size=%u", table->name, family,
              table->info.size);
    if (table->info.num_entries > XT_MAX_ENTRIES)
      failmsg("iptable checkpoint: too many counters",
              "table=%s, family=%d, counters=%d", table->name, family,
...
static void reset_iptables(struct ipt_table_desc *tables, int num_tables,
  ...
  if (fd == -1) {
    ...
    failmsg("iptable: socket(SOCK_STREAM, IPPROTO_TCP) failed", "family=%d",
            family);
  }
  for (int i = 0; i < num_tables; i++) {
    struct ipt_table_desc *table = &tables[i];
...
static void checkpoint_arptables(void) {
  ...
  for (unsigned i = 0; i < sizeof(arpt_tables) / sizeof(arpt_tables[0]); i++) {
    ...
    if (getsockopt(fd, SOL_IP, ARPT_SO_GET_INFO, &table->info, &optlen)) {
      ...
      failmsg("arptable checkpoint: getsockopt(ARPT_SO_GET_INFO) failed",
              "table=%s", table->name);
    }
    debug("arptable checkpoint %s: entries=%d hooks=%x size=%d\n", table->name,
          table->info.num_entries, table->info.valid_hooks, table->info.size);
    if (table->info.size > sizeof(table->replace.entrytable))
      failmsg("arptable checkpoint: table size is too large",
              "table=%s, size=%u", table->name, table->info.size);
    if (table->info.num_entries > XT_MAX_ENTRIES)
      failmsg("arptable checkpoint: too many counters", "table=%s, counters=%u",
    ...
    if (getsockopt(fd, SOL_IP, ARPT_SO_GET_ENTRIES, &entries, &optlen))
      failmsg("arptable checkpoint: getsockopt(ARPT_SO_GET_ENTRIES) failed",
              "table=%s", table->name);
    table->replace.valid_hooks = table->info.valid_hooks;
    table->replace.num_entries = table->info.num_entries;
    table->replace.size = table->info.size;
...
static void reset_arptables() {
  ...
  for (unsigned i = 0; i < sizeof(arpt_tables) / sizeof(arpt_tables[0]); i++) {
    ...
    if (getsockopt(fd, SOL_IP, ARPT_SO_GET_INFO, &info, &optlen))
      failmsg("arptable: getsockopt(ARPT_SO_GET_INFO) failed", "table=%s",
              table->name);
    if (memcmp(&table->info, &info, sizeof(table->info)) == 0) {
      struct arpt_get_entries entries;
      memset(&entries, 0, sizeof(entries));
      ...
      optlen = sizeof(entries) - sizeof(entries.entrytable) + entries.size;
      if (getsockopt(fd, SOL_IP, ARPT_SO_GET_ENTRIES, &entries, &optlen))
        failmsg("arptable: getsockopt(ARPT_SO_GET_ENTRIES) failed", "table=%s",
                table->name);
      if (memcmp(table->replace.entrytable, entries.entrytable,
                 table->info.size) == 0)
        continue;
    ...
    if (setsockopt(fd, SOL_IP, ARPT_SO_SET_REPLACE, &table->replace, optlen))
      failmsg("arptable: setsockopt(ARPT_SO_SET_REPLACE) failed", "table=%s",
              table->name);
  }
  close(fd);
}
...
// ebtables.h:197:19: error: invalid conversion from ‘void*’ to
// ‘ebt_entry_target*’
...
static void checkpoint_ebtables(void) {
  ...
  for (size_t i = 0; i < sizeof(ebt_tables) / sizeof(ebt_tables[0]); i++) {
    ...
    debug("ebtable checkpoint %s: entries=%d hooks=%x size=%d\n", table->name,
          table->replace.nentries, table->replace.valid_hooks,
          table->replace.entries_size);
    if (table->replace.entries_size > sizeof(table->entrytable))
      failmsg("ebtable checkpoint: table size is too large",
              "table=%s, size=%u", table->name, table->replace.entries_size);
  ...
  for (unsigned i = 0; i < sizeof(ebt_tables) / sizeof(ebt_tables[0]); i++) {
    ...
    if (memcmp(&table->replace, &replace, sizeof(table->replace)) == 0) {
      ...
      if (getsockopt(fd, SOL_IP, EBT_SO_GET_ENTRIES, &replace, &optlen))
        failmsg("ebtable: getsockopt(EBT_SO_GET_ENTRIES) failed", "table=%s",
                table->name);
    ...
    if (setsockopt(fd, SOL_IP, EBT_SO_SET_ENTRIES, &table->replace, optlen))
      failmsg("ebtable: setsockopt(EBT_SO_SET_ENTRIES) failed", "table=%s",
              table->name);
  }
  close(fd);
}
...
#if SYZ_EXECUTOR ||                                                            \
    (SYZ_CGROUPS && (SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID ||                 \
                     SYZ_SANDBOX_NAMESPACE || SYZ_SANDBOX_ANDROID))
#include <fcntl.h>
#include <string.h>
#include <sys/mount.h>
...
static void setup_cgroups() {
  ...
  // If we enable all at the same time and one of them fails (b/c of older
  // kernel or not enabled configs), then all will fail.
...
static void drop_caps(void) {
  ...
  if (syscall(SYS_capget, &cap_hdr, &cap_data))
    fail("capget failed");
  // Drop CAP_SYS_PTRACE so that test processes can't attach to parent
  // processes. Previously it lead to hangs because the loop process stopped due
  // to SIGSTOP. Note that a process can always ptrace its direct children,
  // which is enough for testing purposes.
  //
  // A process with CAP_SYS_NICE can bring kernel down by asking for too high
  // SCHED_DEADLINE priority, as the result rcu and other system services that
  // use kernel threads will stop functioning. Some parameters for
  // SCHED_DEADLINE should be OK, but we don't have means to enforce values of
  // indirect syscall arguments. Peter Zijlstra proposed
  // sysctl_deadline_period_{min,max} which could be used to enfore safe limits
  // without droppping CAP_SYS_NICE, but we don't have it yet. See the following
  // bug for details:
...
static int do_sandbox_none() {
  ...
  fprintf(stderr, "----- executor %lld do_sandbox_none at begining\n",
          executor_index);
  ...
  */
  fprintf(stderr, "----- executor %lld do_sandbox_none before fork()\n",
          executor_index);
...
const char *const SELINUX_CONTEXT_UNTRUSTED_APP =
    "u:r:untrusted_app:s0:c512,c768";
const char *const SELINUX_LABEL_APP_DATA_FILE =
...
const gid_t UNTRUSTED_APP_GROUPS[] = {UNTRUSTED_APP_GID, AID_NET_BT_ADMIN,
                                      AID_NET_BT, AID_INET, AID_EVERYBODY};
const size_t UNTRUSTED_APP_NUM_GROUPS =
    sizeof(UNTRUSTED_APP_GROUPS) / sizeof(UNTRUSTED_APP_GROUPS[0]);
...
static void setcon(const char *context) {
  ...
  // N.B.: We cannot reuse this file descriptor, since the target SELinux
  // context
  //       may not be able to read from it.
  close(fd);

  if (bytes_written != (ssize_t)strlen(context))
    failmsg("setcon: could not write entire context", "wrote=%zi, expected=%zu",
            bytes_written, strlen(context));

  // Validate the transition by checking the context
  getcon(new_context, sizeof(new_context));

  if (strcmp(context, new_context) != 0)
    failmsg("setcon: failed to change", "want=%s, context=%s", context,
            new_context);
}
...
static void setfilecon(const char *path, const char *context) {
  ...
  if (strcmp(context, new_context) != 0)
    failmsg("setfilecon: could not set context", "want=%s, got=%s", context,
            new_context);
}
...
// Moreover, a mount can be mounted several times, so we need to try to umount
// in a loop. Moreover, after umount a dir can become non-empty again, so we
// need another loop. Moreover, a mount can be re-mounted as read-only and then
// we will fail to make a dir empty.
static bool remove_dir(const char *dir) {
  int iter = 0;
  DIR *dp = 0;
  // char buf[50+FILENAME_MAX];
retry:
#if SYZ_EXECUTOR || !SYZ_SANDBOX_ANDROID
#if SYZ_EXECUTOR
  ...
  if (dp == NULL) {
    if (errno == EMFILE) {
      ...
      // Ideally we somehow prevent test processes from messing with parent
      // processes. But full sandboxing is expensive, so let's ignore this error
      // for now.
      fprintf(stderr, "opendir(%s) failed due to NOFILE, exiting", dir);
      return false;
    }
    fprintf(stderr, "opendir(%s) failed", dir);
    return false;
  }
  struct dirent *ep = 0;
  while ((ep = readdir(dp))) {
    ...
    if (lstat(filename, &st)) {
      fprintf(stderr, "lstat(%s) failed", filename);
      return false;
    }
    if (S_ISDIR(st.st_mode)) {
      if (remove_dir(filename) == false)
        return false;
      continue;
    }
    int i;
    for (i = 0; i < 100; i++) {
      ...
      if (!flag_sandbox_android) {
        ...
        if (umount2(filename, MNT_DETACH | UMOUNT_NOFOLLOW)) {
          fprintf(stderr, "umount(%s) failed", filename);
          return false;
        }
#if SYZ_EXECUTOR
      }
  ...
  for (int i = 0;; i++) {
      ...
      if (errno == EBUSY) {
        ...
        if (!flag_sandbox_android) {
          ...
          if (umount2(dir, MNT_DETACH | UMOUNT_NOFOLLOW)) {
            fprintf(stderr, "umount(%s) failed", dir);
            return false;
          }
#if SYZ_EXECUTOR
        }
    ...
    fprintf(stderr, "rmdir(%s) failed", dir);
    return false;
  }
  return true;
}
...
#if SYZ_EXECUTOR || SYZ_REPEAT && (SYZ_NET_RESET || __NR_syz_mount_image ||    \
                                   __NR_syz_read_part_table)
...
static void setup_fault() {
  static struct {
    ...
    bool fatal;
      ...
      // and we did not check all of them in host.checkFaultInjection, so we
      // ignore errors.
  ...
  for (i = 0; i < sizeof(files) / sizeof(files[0]); i++) {
    if (!write_file(files[i].file, files[i].val)) {
      ...
      if (files[i].fatal)
        failmsg("failed to write fault injection file", "file=%s",
                files[i].file);
    }
  }
}
...
static void setup_sysctl() {
  ...
  // Kernel has support for setting sysctl's via command line since
  // 3db978d480e28 (v5.8).
  struct {
    const char *name;
    const char *data;
      ...
      // nmi_check_duration() prints "INFO: NMI handler took too long" on slow
      // debug kernels.
      ...
      // This is to restrict effects of recursive exponential mounts, for
      // details see "mnt: Add a per mount namespace limit on the number of
      // mounts" commit.
      ...
      // We always want to prefer killing the allocating test process rather
      // than somebody else (sshd or another random test process).
      ...
      // and setting cad_pid to the current pid (transient "syz-executor setup")
      // makes it a no-op. For context see:
      // https://groups.google.com/g/syzkaller-bugs/c/WqOY4TiRnFg/m/6P9u8lWZAQAJ
...
#if GOARCH_s390x
#include <sys/mman.h>
// Ugly way to work around gcc's "error: function called through a
// non-compatible type". The macro is used in generated C code.
#define CAST(f)                                                                \
  ({                                                                           \
    void *p = (void *)f;                                                       \
    p;                                                                         \
...
// 2. Register as an application responsible for wireless medium simulation by
// executing
//    HWSIM_CMD_REGISTER. This is a preq-requisite for the following step. After
//    HWSIM_CMD_REGISTER is executed, mac80211_hwsim stops simulating a perfect
//    medium. It is also important to note that this command registers a
//    specific socket, not a netlink port.
// 3. Inject a frame to the required interface by executing HWSIM_CMD_FRAME.
// 4. Close the socket. mac80211_hwsim will detect this and return to perfect
// medium simulation.
//
// Note that we cannot (should not) open a socket, register it once and then use
// it for frame injection throughout the lifetime of a proc. When some socket is
// registered, mac80211_hwsim does not broadcast frames to all interfaces
// itself. As we do not perform this activity either, a permanently registered
...
static int hwsim_inject_frame(struct nlmsg *nlmsg, int sock, int hwsim_family,
                              uint8 *mac_addr, uint8 *data, int len) {
  struct genlmsghdr genlhdr;
...
static long syz_80211_join_ibss(volatile long a0, volatile long a1,
  ...
  uint8 *ssid = (uint8 *)a1;
  int ssid_len = (int)a2;
  int mode = (int)a3; // This parameter essentially determines whether it will
                      // perform a scan
  ...
  if (mode == WIFI_JOIN_IBSS_NO_SCAN) {
    ...
    if (ret < 0) {
      debug("syz_80211_join_ibss: await_ifla_operstate failed for %.32s, ret "
            "%d\n",
            interface, ret);
      return -1;
    }
