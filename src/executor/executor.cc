...
// tao modified
// 16MB: the metadata (fsMd) dump of a full merge_view tree plus call output
// and eBPF events must fit; 1MB overflows once the tree grows beyond a few
// thousand entries. Must match ipc.go outmem and qemu.go ivshmem size.
...
struct execute_req {
  ...
  uint64 file_ops_offset;
  uint64 file_ops_size;
};
...
// Calibration of the monotonic-ns -> raw-TSC affine map.
// Events captured by the eBPF tracer use bpf_ktime_get_ns() (monotonic ns),
// while call windows use rdtsc(). Both are linear functions of real time on
// the same CPU, so one anchor pair + the ratio lets us convert either way.
double tsc_ns_ratio = 1.0;
...
uint64_t tsc_ns_to_global(uint64_t ns) {
  uint64_t tsc = tsc_anchor_tsc + (uint64_t)((double)(ns - tsc_anchor_ns) / tsc_ns_ratio);
  return tsc - (uint64_t)tsc_offset;
}

#else

static void calibrate_tsc(void) {}
uint64_t tsc_ns_to_global(uint64_t ns) { return ns; }
...
static bool is_valid_path(const char *path) {
    ...
    // 检查路径是否以 "./" 或 "/" 开头（常见的路径格式）
    return strncmp(path, "./", 2) == 0 || path[0] == '/';
}
...
// wait servers before current node are already setupped
  ...
  if (!strcmp(dfs_name, "hmdfs")) {
    sprintf(cwdbuf, "/mnt/hmdfs/100/non_account");
  }
  if (chdir(cwdbuf)) {
    char outmsg[1024];
...
void receive_handshake() {
  ...
  // agent can pick its own CID/IP. For other DFSes only node 0 configures.
  if (executor_index == 0 || !strcmp(dfs_name, "hmdfs")) {
    ...
    if (!strcmp(dfs_name, "hmdfs")) {
      ...
      snprintf(cmdbuf, sizeof(cmdbuf), "/root/%s-config.sh %s %s 2>&1",
               dfs_name, init_ip, dfs_setup_params);
    }
    FILE *cmd = popen(cmdbuf, "r");
    if (cmd != NULL) {
#if MDEBUG
...
uint64 file_ops_offset = 0;
uint64 file_ops_size = 0;
...
uint64 receive_execute() {
  ...
  file_ops_size = req.file_ops_size;
  ...
  if(!strcmp(dfs_name, "hmdfs")) {
    snprintf(cwdbuf, 199, "/mnt/hmdfs/100/non_account");
  } else {
    snprintf(cwdbuf, 199, "/root/%s-client/dfs-%lld-%lld", dfs_name, req.pid,
           execution_index);
  }
  
  if (executor_index == server_num && strcmp(dfs_name, "hmdfs")) {
  ...
  } else if(executor_index == 0 && !strcmp(dfs_name, "hmdfs")) {
      snprintf(lastcwdbuf, 199, "/mnt/hmdfs/100/non_account");
      //fprintf(stderr, "empty dir: %s\n", lastcwdbuf);
      //empty_dir(lastcwdbuf);
      execCtl->tmpDirEstablished = 1;
  } else {
...
void reply_execute(int status, int iter) {
  ...
  if (is_restarting)
    is_restarting = 0;
  // release barrier: all output writes (reply, metadata, trace) must be
...
void write_call_output(thread_t *th, bool finished, cover_t *usp_covers) {
  ...
  if (call1->call == NULL) {
    ...
    case 196: // flistxattr (196)
      ...
      break;
    case 217: // getdents64 (217) : Success: number of bytes, End of directory: 0,
             // Error: -1
      if (th->res > 0) {
        struct dirent *startp = (struct dirent *)th->args[1];
        char *infoc = (char *)info;
        char *endp = (char *)startp + th->res;
        size_t used = 0;
        // getdents64 packs entries compactly via d_reclen (not
        // sizeof(struct dirent)), so advance by d_reclen.
        for (struct dirent *cur = startp;
             cur->d_reclen > 0 && (char *)cur + cur->d_reclen <= endp;
             cur = (struct dirent *)((char *)cur + cur->d_reclen)) {
          size_t name_len = strlen(cur->d_name);
          if (used + name_len + 2 > INFOLIMIT)
            break;
          memcpy(infoc + used, cur->d_name, name_len);
          used += name_len;
          infoc[used++] = ';';
        }
        if (used + 2 <= INFOLIMIT)
...
void write_extra_output() {
  ...
  // Placeholder header fields below keep the 192-byte callReply wire format
  // uniform: Go skips them for extra records (index == -1 -> continue in
  // parseClientOutput), so only the counts and payload below are consumed.
  write_output(-1);                           // infotype: no info payload
  uint32 *info = occupy_nbytes(INFOLIMIT);    // info[144], explicitly zeroed
  memset(info, 0, INFOLIMIT);
  write_output_64(0);                         // stime placeholder
  write_output_64(0);                         // etime placeholder
  ...
  completed++;
  write_completed(completed);
  write_output_64(0);                         // res placeholder (Go reads it)
#endif
}
...
void write_metadata(int iter) {
  ...
  if (output_pos &&
    ...
    for (uint64 i = server_num; i < vm_count; i++) {
      uint64 wait_start = current_time_ms();
      while (!(execCtl->executionsFinish[i])) {
        if (current_time_ms() - wait_start > program_timeout_ms) {
          fprintf(stderr,
                  "executor %lld: timeout waiting client %llu executionsFinish\n",
                  executor_index, i);
          break;
        }
        sleep_ms(1);
    ...
    if(!strcmp(dfs_name, "hmdfs"))
        snprintf(cwdbuf, 239, "/mnt/hmdfs/100/non_account/merge_view");
