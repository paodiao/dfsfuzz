...
extern uint64 is_dfs_client, executor_index, server_num, execution_index,
    in_userspace, is_restarting;
void collect_srv_cover(bool wait);
void reinit_cov_shms();

#if GOOS_akaros
#include <parlib/parlib.h>
static void recover(void) { _longjmp(segv_env, 1); }
#endif
...
static void kill_and_wait(int pid, int *status) {
  ...
  while (waitpid(-1, status, 0) != pid) {
    ...
    SYZ_REPEAT && SYZ_EXECUTOR_USES_FORK_SERVER || __NR_syz_usb_connect ||     \
    __NR_syz_usb_connect_ath9k || __NR_syz_sleep_ms
static void sleep_ms(uint64 ms) { usleep(ms * 1000); }
#endif

#if SYZ_EXECUTOR || SYZ_THREADED ||                                            \
    SYZ_REPEAT && SYZ_EXECUTOR_USES_FORK_SERVER || SYZ_LEAK
#include <time.h>
...
#if GOOS_akaros || GOOS_netbsd || GOOS_freebsd || GOOS_darwin ||               \
    GOOS_openbsd || GOOS_test
#if (SYZ_EXECUTOR || SYZ_REPEAT) && SYZ_EXECUTOR_USES_FORK_SERVER &&           \
    (SYZ_EXECUTOR || SYZ_USE_TMP_DIR)
#include <dirent.h>
#include <errno.h>
#include <stdio.h>
#include <string.h>
...
// We need to prevent the compiler from unrolling the while loop by using the
// gcc's noinline attribute because otherwise it could trigger the compiler
// warning about a potential format truncation when a filename is constructed
// with help of snprintf. This warning occurs because by unrolling the while
// loop, the snprintf call will try to concatenate 2 buffers of length
// FILENAME_MAX and put the result into a buffer of length FILENAME_MAX which is
// apparently not possible. But this is no problem in our case because file and
// directory names should be short enough and fit into a buffer of length
// FILENAME_MAX.
...
static int inject_fault(int nth) { return 0; }
#endif
...
#if SYZ_EXECUTOR
static int fault_injected(int fail_fd) { return 0; }
#endif
#endif
...
static void thread_start(void *(*fn)(void *), void *arg) {
  ...
  // Clone can fail spuriously with EAGAIN if there is a concurrent execve in
  // progress. (see linux kernel commit 498052bba55ec). But it can also be a
  // true limit imposed by cgroups. In one case we want to retry infinitely, in
  // another -- fail immidiately...
  int i = 0;
  for (; i < 100; i++) {
...
#if GOOS_freebsd || GOOS_darwin || GOOS_netbsd || GOOS_openbsd ||              \
    GOOS_akaros || GOOS_test
#if SYZ_EXECUTOR || SYZ_THREADED
...
static void csum_inet_update(struct csum_inet *csum, const uint8 *data,
                             size_t length) {
  if (length == 0)
    return;
...
static uint16 csum_inet_digest(struct csum_inet *csum) { return ~csum->acc; }
#endif
...
// extern uint64 is_dfs_client, executor_index, server_num, execution_index,
// in_userspace;
extern char *dfs_name, *input_data;
extern uint64 skip_handshake;
...
static void loop() {
  ...
  // Skip handshake when crash consistency or failure injection
  if (!skip_handshake) {
    reply_handshake();
  } else {
	skip_handshake = 0;
  ...
  // For akaros we do exec in the child process because new threads can't be
  // created in the fork child. Thus we proxy input program over the child_pipe
...
#endif
  int iter = 0;
  //int shmid = shmget(IPC_PRIVATE, sizeof(uint32 *) * 2, IPC_CREAT | 0600);
  //uint32 **shmaddr = (uint32 **)shmat(shmid, NULL, 0);
  //int shmid_need_reboot = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0600);
  //shm_need_reboot = (int *)shmat(shmid_need_reboot, NULL, 0);
  //*shm_need_reboot = 0;
  ...
  for (;; iter++) {
    ...
    // Create a new private work dir for this test (removed at the end of the
    // loop).
    // tao modified
    /*
    char tmpbuf[64];
            char cwdbuf[1024];//32
    if(getcwd(tmpbuf, 64) == NULL)
        debug("getcwd error\n");
            sprintf(cwdbuf, "%s/%d", tmpbuf, iter);
    debug("cwdbuf %s\n", cwdbuf);
    ...
    */
    ...
    // fprintf(stderr, "----- before reset_loop, is_restarting %lld, restarting
    // tag bit %d\n", is_restarting, output_ctl_pos->executionFinished);

#if SYZ_HAVE_RESET_LOOP
    reset_loop();
#endif

    // fprintf(stderr, "----- after reset_loop, is_restarting %lld, restarting
    // tag bit %d\n", is_restarting, output_ctl_pos->executionFinished);
    int time1 = time_now();
#if SYZ_EXECUTOR
    receive_execute();
#endif
    int time2 = time_now();
    // fprintf(stderr, "----- execute %lld receive_execute %lld restarting tag
    // bit %d is_retarting %lld\n", executor_index, execution_index,
    // output_ctl_pos->executionFinished, is_restarting);

    /*
if (!is_dfs_client) {
  // sleep(2);
  reply_execute(0, execution_index);
  continue;
}
    */
    ...
    if (pid == 0) {
      ...
      snprintf(cwdbuf, 199, "/root/%s-client/dfs-%lld-%lld", dfs_name, req_pid,
      execution_index);

      if (executor_index == server_num) {
          if (execution_index > 0) {
              snprintf(lastcwdbuf, 199, "/root/%s-client/dfs-%lld-%lld",
      dfs_name, req_pid, execution_index-1); fprintf(stderr, "remove dir: %s\n",
      lastcwdbuf); remove_dir(lastcwdbuf);
          }
      ...
      cwdbuf); if (chdir(cwdbuf)) { char errbuf[500]; snprintf(errbuf, 499, "%d
      executor xx %lld failed to chdir %s, %s \n", 123, executor_index, cwdbuf,
      strerror(errno)); fprintf(stderr, "%s", errbuf); fail("executor failed to
      chdir");
      }
      */

#endif
      // fprintf(stderr, "----- fork child, is_restarting %lld, restarting tag
      // bit %d\n", is_restarting, output_ctl_pos->executionFinished);

      // Initialize output_pos in shm
      //shmaddr[0] = 0;

	  //fprintf(stderr, "executor %lld after shmaddr\n", executor_index);
	  ...
	  //fprintf(stderr, "executor %lld after setup_test\n", executor_index);
...
#endif
      int time11 = time_now();
	  fprintf(stderr, "executor %lld before execute_one\n", executor_index);
      execute_one();
      int time12 = time_now();
      fprintf(stderr, "execute_one time: %d, %d\n", time11 - time2,
              time12 - time11);

      // clt_finish();
      // write server cover here. Because dedup_table should be owned by one
      // execution.
      collect_srv_cover(true);

      // return output_pos to parent process
      // write_output(0);
      //shmaddr[0] = output_pos;
      /*
      close(output_pos_fd[0]);
      extern int errno;
      errno = 0;
      ret = write(output_pos_fd[1], &output_pos, sizeof(output_pos));
      fprintf(stderr, "output_pos write %d %s\n", ret, strerror(errno));
      */
    ...
    // so it would require a loop with status analysis and timeout
    // recalculation. SIGCHLD should also unblock the usleep below, so the spin
    // loop should be as efficient as sigtimedwait.
    int status = 0;
    uint64 start = current_time_ms();
#if SYZ_EXECUTOR && SYZ_EXECUTOR_USES_SHMEM
    ...
    for (;;) {
      ...
      // and execution time of each syscall is bounded by syscall_timeout_ms
      // (~50ms), this backup watchdog is necessary and its performance is
      // important. The problem is that exit in the test processes can fail
      // (sic). One observed scenario is that the test processes prohibits
      ...
      // uint64 min_timeout_ms = program_timeout_ms * 10000 / 5;
      // uint64 inactive_timeout_ms = syscall_timeout_ms * 10000;
      ...
      // fprintf(stderr, "program_timeout_ms %lld, min_timeout_ms %lld,
      // inactive_timeout_ms %lld\n", program_timeout_ms, min_timeout_ms,
      // inactive_timeout_ms);
      if ((now - start < program_timeout_ms) &&
          (now - start < min_timeout_ms ||
           now - last_executed < inactive_timeout_ms))
        continue;
#else
...
#endif
      fprintf(stderr,
              "executor %lld killing hanging pid %d, execution time %lld\n",
              executor_index, pid, current_time_ms() - start);
      kill_and_wait(pid, &status);
      fprintf(stderr, "executor %lld finishes killing\n", executor_index);
      break;
    }
    int time3 = time_now();
#if SYZ_EXECUTOR
    ...
    // read output_pos from child process
    /*
    close(output_pos_fd[1]);
    extern int errno;
    errno = 0;
    int ret = read(output_pos_fd[0], &output_pos, sizeof(output_pos));
    */
    //output_pos = shmaddr[0];
    //fprintf(stderr, "-----executor %lld output_pos %p\n", executor_index, output_pos);

    // fprintf(stderr, "----- executor %lld reply_execute() in loops\n",
    // executor_index);
    //if (shm_need_reboot[0]) {
    //  process_before_reboot();
    //} else {
      reply_execute(0, execution_index);
    //}
    //is_restarting = 0;
    int time4 = time_now();
#endif
    fprintf(stderr, "time breakdown %lld: %d, %d, %d\n", executor_index,
...
#if SYZ_THREADED || SYZ_REPEAT || SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID ||    \
