...
type Stats struct {
	...
	corpusCliSignal     Stat
	corpusSrvSignal     Stat
	maxSignal           Stat
	corpusFailSignal	Stat
...
func (mgr *Manager) initStats() {
	...
	prometheus.Register(promauto.NewGaugeFunc(prometheus.GaugeOpts{
		...
		func() float64 { return float64(mgr.stats.corpusCliCover.get() + mgr.stats.corpusSrvCover.get()) },
	))
	prometheus.Register(promauto.NewGaugeFunc(prometheus.GaugeOpts{
...
func (stats *Stats) all() map[string]uint64 {
	m := map[string]uint64{
		...
		"client coverage":   stats.corpusCliCover.get(),
		"server coverage":   stats.corpusSrvCover.get(),
		"filtered coverage": stats.corpusCoverFiltered.get(),
		"client signal":     stats.corpusCliSignal.get(),
		"server signal":     stats.corpusSrvSignal.get(),
