...
type WorkTriage struct {
	...
	flags        ProgTypes
	triageClient bool
	// triageDag marks a DAG-driven corpus entry: the coverage gate,
	// re-execution verification and minimization are skipped.
	triageDag bool
}
...
func (wq *WorkQueue) enqueue(item interface{}) {
	...
	case *WorkFSmash:
		wq.fsmash = append(wq.fsmash, item)
	default:
...
func (wq *WorkQueue) dequeue() (item interface{}) {
	...
	} else if len(wq.fsmash) != 0 {
		last := len(wq.fsmash) - 1
		item = wq.fsmash[last]
		wq.fsmash = wq.fsmash[:last]
	}
	wq.mu.Unlock()
	if wantCandidates {
		select {
