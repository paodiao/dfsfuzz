...
func generateInput(target *prog.Target, rs rand.Source, ncalls, sizeSig int) (inp InputTest) {
	inp.p, _ = target.Generate(rs, ncalls, target.DefaultChoiceTable(), nil, false, nil, false, &prog.Hmdfs_config{}, 0)
	var raw []uint32
	for i := 1; i <= sizeSig; i++ {
