...
func testImage(hostAddr string, args *checkArgs, hostFuzzerCheckerCmd string) {
	...
	conn.Close()
	if _, err := checkMachine(args, hostFuzzerCheckerCmd); err != nil {
...
func checkMachine(args *checkArgs, hostFuzzerCheckerCmd string) (*rpctype.CheckArgs, error) {
	...
	features, err := host.Check(args.target, hostFuzzerCheckerCmd)
	if err != nil {
		return nil, err
	}
...
func checkRevisions(args *checkArgs) error {
	...
	executorArgs := strings.Split(args.ipcConfig.Executor[0], " ")
...
func checkSimpleProgram(args *checkArgs, features *host.Features) error {
	...
	if err := host.Setup(args.target, features, args.featureFlags, args.ipcConfig.Executor[0]); err != nil {
