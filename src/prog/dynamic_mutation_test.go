...
// TestInsertCallFromPatternTrace replays the first broken-ref seed with the
// ipf-trace diagnostics sink attached, printing the full insertion trace:
// pattern choice, per-node insertPos/fd decisions, generated op groups and
// the resulting call layout. Temporary localization aid.
func TestInsertCallFromPatternTrace(t *testing.T) {
	target := smokeTarget(t)
	sCalls := hmdfsSmokeSpecialCalls(t, target)
	cfg := &Hmdfs_config{
		Cids:     []string{"c1", "c2", "c3"},
	...
	const seed = 1
	prev := diagSink
	SetDiagSink(func(format string, args ...interface{}) {
		t.Logf("[diag] "+format, args...)
	})
	defer SetDiagSink(prev)

	for _, seedType := range []string{"fileops", "inodeops"} {
		lcs := NewLayeredChoiceStrategy(seedType, cfg, target)
		ps := Clones(patternSingleOpenBase(target))
		r := newRand(ps[0].Target, rand.NewSource(seed))
		insertCallFromPattern(ps, r, sCalls, cfg, lcs, seedType)
		for i, p := range ps {
			if p.HasBrokenRefs() {
				t.Logf("seedType=%s ps[%d] BROKEN after insert:\n%s", seedType, i, p.DumpRefDiagnosis())
			}
		}
	}
}
