...
func TestHintsData(t *testing.T) {
	...
	for _, test := range tests {
		ps, err := target.Deserialize([][]byte{[]byte(fmt.Sprintf("test$hint_data(&AUTO=\"%v\")", test.in))}, Strict)
		if err != nil {
			t.Fatal(err)
		}
		p := ps[0]
		var got []string
		MutateWithHints([]*Prog{p}, 0, 0, test.comps, func(ps []*Prog) {
			newP := ps[0]
...
func BenchmarkHints(b *testing.B) {
	...
	r := newRand(target, rs)
	ct := target.DefaultChoiceTable()
	p, _ := target.Generate(rs, 30, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			for i := range p.Calls {
				MutateWithHints([]*Prog{p}, 0, i, comps[i], func(ps []*Prog) {})
			}
		}
	})
}
