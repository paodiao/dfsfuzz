...
import (
	...
	"monarch/prog"
	"monarch/sys/targets"
	_ "monarch/sys/test/gen" // import the target we use for fuzzing
)

func FuzzDeserialize(data []byte) int {
	ps0, err0 := fuzzTarget.Deserialize([][]byte{data}, prog.NonStrict)
	ps1, err1 := fuzzTarget.Deserialize([][]byte{data}, prog.Strict)
	if len(ps0) == 0 {
		if len(ps1) != 0 {
	...
	if len(ps1) != 0 {
		...
		if !bytes.Equal(data0, ps1[0].Serialize()) {
			panic("got different data")
		}
	}
	ps2, err2 := fuzzTarget.Deserialize([][]byte{data0}, prog.NonStrict)
	if err2 != nil {
		panic(fmt.Sprintf("failed to parse serialized: %v\n%s", err2, data0))
	}
	if !bytes.Equal(data0, ps2[0].Serialize()) {
		panic("got different data")
	}
	p3 := p0.Clone()
	...
	p3.Mutate(rand.NewSource(0), 3, fuzzChoiceTable, nil, nil, 0, false, false, &prog.Hmdfs_config{}, 0)
	return 0
}
