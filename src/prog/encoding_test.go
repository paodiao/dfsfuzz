...
func TestCallSetRandom(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		const ncalls = 10
		p, _ := target.Generate(rs, ncalls, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
...
func TestSerializeDeserializeRandom(t *testing.T) {
	testEachTargetRandom(t, func(t *testing.T, target *Target, rs rand.Source, iters int) {
		...
		for i := 0; i < iters; i++ {
			p0, _ := target.Generate(rs, 10, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
			if ok, _, _ := testSerializeDeserialize(t, p0, data0, data1); ok {
				continue
			}
			minPs, _ := Minimize([]*Prog{p0}, -1, 0, false, 0, func(minPs []*Prog, _ int) bool {
				p1 := minPs[0]
...
func testSerializeDeserialize(t *testing.T, p0 *Prog, data0, data1 []byte) (bool, int, int) {
	...
	ps1, err := p0.Target.Deserialize([][]byte{serialized}, NonStrict)
	if err != nil {
		t.Fatal(err)
	}
	p1 := ps1[0]
...
func TestSerializeCallProps(t *testing.T) {
	...
	for _, test := range tests {
		ps, err := target.Deserialize([][]byte{[]byte(test.prog)}, Strict)
...
func TestDeserializeComments(t *testing.T) {
	target := initTargetTest(t, "test", "64")
	ps, err := target.Deserialize([][]byte{[]byte(`
# comment1
# comment2
serialize0(0x0)
serialize0(0x0)
...
func TestSerializeMetaRoundTrip(t *testing.T) {
	testEachTargetRandom(t, func(t *testing.T, target *Target, rs rand.Source, iters int) {
		markers := []struct {
			...
			{"stash", func(p *Prog) { p.IsStash = true }},
			{"dcache", func(p *Prog) { p.IsDCache = true }},
			{"fileops", func(p *Prog) { p.IsFileOps = true }},
			{"inodeops", func(p *Prog) { p.IsInodeOps = true }},
		}
		for _, m := range markers {
			p0, _ := target.Generate(rs, 10, target.DefaultChoiceTable(), nil, false, nil, false, &Hmdfs_config{}, 0)
			m.apply(p0)
			data := p0.Serialize()
			if !bytes.Contains(data, []byte("# prog-meta:")) {
				t.Fatalf("meta line missing for %v:\n%s", m.name, data)
			}
			ps, err := target.Deserialize([][]byte{data}, NonStrict)
			if err != nil {
				t.Fatal(err)
			}
			p1 := ps[0]
			...
			if !got {
				t.Fatalf("marker %v lost in round-trip\nprog:\n%s", m.name, data)
			}
		}
	})
}
