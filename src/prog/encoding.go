...
func (p *Prog) serialize(verbose bool) []byte {
	...
	if meta := p.metaLine(); meta != "" {
		ctx.printf("# prog-meta: %s\n", meta)
	}
...
// progMeta is the serialized subset of Prog metadata. Only the seed-type
// markers are persisted: they are consumed by mutateHmdfs dispatch after a
// corpus round-trip (fuzzer restart), while SyncIdx/position tables have no
// consumers outside generation and are left out. The meta line is emitted
// only when at least one marker is set, so plain progs keep the legacy text
// format unchanged.
type progMeta struct {
	IsStash    bool `json:"is_stash"`
	IsDCache   bool `json:"is_dcache"`
	IsFileOps  bool `json:"is_file_ops"`
	IsInodeOps bool `json:"is_inode_ops"`
}

func (p *Prog) metaLine() string {
	m := progMeta{
		IsStash:    p.IsStash,
		IsDCache:   p.IsDCache,
		IsFileOps:  p.IsFileOps,
		IsInodeOps: p.IsInodeOps,
	}
	if !m.IsStash && !m.IsDCache && !m.IsFileOps && !m.IsInodeOps {
		return ""
	}
...
func (target *Target) Deserialize(datas [][]byte, mode DeserializeMode) ([]*Prog, error) {
	defer func() {
		if err := recover(); err != nil {
			panic(fmt.Errorf("%v\ntarget: %v/%v, rev: %v, mode=%v, prog:\n%q",
				err, target.OS, target.Arch, GitRevision, mode, custom.Concat(datas...)))
		}
	}()
	var progs []*Prog
...
func (p *parser) parseProg() (*Prog, error) {
	...
	for p.Scan() {
		...
		if p.Char() == '#' {
			...
			if strings.HasPrefix(p.comment, "prog-meta:") {
				var m progMeta
				if err := json.Unmarshal([]byte(strings.TrimPrefix(p.comment, "prog-meta:")), &m); err == nil {
					prog.IsStash = m.IsStash
					prog.IsDCache = m.IsDCache
					prog.IsFileOps = m.IsFileOps
					prog.IsInodeOps = m.IsInodeOps
				}
				p.comment = ""
			}
			continue
...
func CallSet(datas ...[]byte) (map[string]struct{}, int, error) {
...
/******************************/
/*
	//buf, fd, string, constant
	switch a.(type) {
		*ConstArg: ctx.printf("0x%x", a.Val)
		*PointerArg: a.
		*DataArg: a.symc3serialize(ctx)
	}
	ctx.arg(a)
*/

func (p *Prog) SerializeForSymc3() string {
	p.debugValidate()
	ctx := &serializer{
		target:   p.Target,
		buf:      new(bytes.Buffer),
		vbuf:     new(bytes.Buffer),
		symc3buf: new(bytes.Buffer),
		vars:     make(map[*ResultArg]int),
	...
	for _, c := range p.Calls {
		/*
			if strings.Contains(c.Meta.Name, "syz_failure") {
				continue
			}
		*/
		if c.Ret != nil && len(c.Ret.uses) != 0 {
			ctx.symc3printf("v%v = ", ctx.allocVarID(c.Ret))
		}
		ctx.symc3printf("syscall(SYS_%v, ", strings.Split(c.Meta.Name, "$")[0])
		for i, a := range c.Args {
			ctx.buf.Reset()
			if IsPad(a.Type()) {
				continue
			...
			switch a.(type) {
			case *ConstArg:
				ctx.symc3printf("%v", a.(*ConstArg).Val)
			case *PointerArg:
				a.(*PointerArg).symc3serialize(ctx)
			case *DataArg:
				a.(*DataArg).symc3serialize(ctx)
			case *ResultArg:
				a.(*ResultArg).symc3serialize(ctx)
			case *UnionArg:
				a.(*UnionArg).symc3serialize(ctx)
			case *GroupArg:
				a.(*GroupArg).symc3serialize(ctx)
			default:
				fmt.Printf("no corresponding type: %T\n", a)
			}
		}
		ctx.symc3printf(");\n")
	}
	...
	if ctx.correct {
		return fmt.Sprintf("%v%v\n", ctx.vbuf, ctx.symc3buf)
	} else {
		return ""
	}
}

func (a *DataArg) symc3serialize(ctx *serializer) {

	typ := a.Type().(*BufferType)
	if a.Dir() == DirOut {
		vid := ctx.allocVarID(nil)
		ctx.vprintf("v%v[%v];\n", vid, a.Size())
		ctx.symc3printf("v%v", vid)
		return
	}
	...
	//TODO
	symc3serializeData(ctx.buf, data, isReadableDataType(typ))
	if typ.Varlen() && sz != len(data) {
		ctx.printf("/%v", sz)
	}
	//
...
func (a *PointerArg) symc3serialize(ctx *serializer) {
	if a.IsSpecial() {
		vid := ctx.allocVarID(nil)
		ctx.vprintf("v%v[0];\n", vid)
		ctx.symc3printf("v%v", vid)
	...
	data1, ok1 := a.Res.(*DataArg)
	data2, ok2 := a.Res.(*GroupArg)
	if (ok1 && data1.Dir() == DirOut) || (ok2 && data2.Dir() == DirOut) {
		return
	}

	str := ctx.buf.String()
	vid := 0
	vid, ok := ctx.varMap[str]
	if !ok {
		vid = ctx.allocVarID(nil)
		if len(str) == 0 {
			ctx.vprintf("v%v[0];\n", vid)
...
func (a *ResultArg) symc3serialize(ctx *serializer) {
	if len(a.uses) != 0 {
		ctx.symc3printf("<v%v=>", ctx.allocVarID(a))
	}
	if a.Res == nil {
		ctx.symc3printf("%v", int64(a.Val))
		return
	}
	id, ok := ctx.vars[a.Res]
	if !ok {
		...
		ctx.symc3printf("v%v", id)
		if a.OpDiv != 0 {
			ctx.symc3printf("/%v", a.OpDiv)
		}
		if a.OpAdd != 0 {
			ctx.symc3printf("+%v", a.OpAdd)
		}
	}
}
...
func (a *GroupArg) symc3serialize(ctx *serializer) {
	if a.Dir() == DirOut {
		vid := ctx.allocVarID(nil)
		ctx.vprintf("v%v[%v];\n", vid, a.Size())
		ctx.symc3printf("v%v", vid)
...
func (a *UnionArg) symc3serialize(ctx *serializer) {
	//typ := a.Type().(*UnionType)
	//ctx.printf("@%v", typ.Fields[a.Index].Name)
	if !ctx.verbose && isDefault(a.Option) {
		return
	...
	//ctx.printf("=")
	ctx.symc3arg(a.Option)
}

func (ctx *serializer) symc3arg(arg Arg) {
	if arg == nil {
		ctx.printf("nil")
		return
	}
	switch arg.(type) {
	case *ConstArg:
		ctx.printf("\\x%02x", arg.(*ConstArg).Val)
	case *PointerArg:
		arg.(*PointerArg).symc3serialize(ctx)
	case *DataArg:
		arg.(*DataArg).symc3serialize(ctx)
	case *ResultArg:
		arg.(*ResultArg).symc3serialize(ctx)
	case *UnionArg:
		arg.(*UnionArg).symc3serialize(ctx)
	case *GroupArg:
		arg.(*GroupArg).symc3serialize(ctx)
	default:
		fmt.Printf("no corresponding type: %T\n", arg)
	}
}
...
func (ctx *serializer) vprintf(text string, args ...interface{}) {
	fmt.Fprintf(ctx.vbuf, text, args...)
}

func (ctx *serializer) symc3printf(text string, args ...interface{}) {
	fmt.Fprintf(ctx.symc3buf, text, args...)
}

func symc3serializeData(buf *bytes.Buffer, data []byte, readable bool) {
	// log.Logf(0, "symc3serializeData 1: %v\n", data)
	if readable && len(data) > 1 && data[len(data)-1] == 0x0 {
		data = data[:len(data)-1]
	}
	for _, b := range data {
		if isPrintable(b) {
...
func Serialize2Str(data []byte) string {

	new_data := make([]byte, 0)
	for _, char := range data {
		if isPrintable(char) {
			new_data = append(new_data, char)
		} else {
			lo, hi := byteToHex(char)
			new_data = append(new_data, []byte{'\\', 'x', hi, lo}...)
		}
	}
