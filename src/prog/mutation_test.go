	...
	next:
		for i := 0; i < iters; i++ {
			...
			for try := 0; try < 20; try++ {
				p1.Mutate(rs, 10, ct, nil, nil, 0, false, false, &Hmdfs_config{}, 0)
				data := p.Serialize()
				...
				if _, err := target.Deserialize([][]byte{data1}, NonStrict); err != nil {
...
func TestMutateCorpus(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		p1, _ := target.Generate(rs, 10, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
		p1.Mutate(rs, 10, ct, [][]*Prog{corpus}, nil, 0, false, false, &Hmdfs_config{}, 0)
	}
}
...
func BenchmarkMutate(b *testing.B) {
	...
	ct := target.DefaultChoiceTable()
	const progLen = 30
	p, _ := target.Generate(rand.NewSource(0), progLen, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		rs := rand.NewSource(0)
		for pb.Next() {
			p.Clone().Mutate(rs, progLen, ct, nil, nil, 0, false, false, &Hmdfs_config{}, 0)
		}
	})
}
...
func BenchmarkGenerate(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		rs := rand.NewSource(0)
		for pb.Next() {
			_, _ = target.Generate(rs, progLen, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
		}
	})
}
...
func runMutationTests(t *testing.T, tests [][2]string, valid bool) {
	...
	for ti, test := range tests {
		...
		t.Run(fmt.Sprint(ti), func(t *testing.T) {
			...
			for i := 0; i < iters; i++ {
				p1 := p.Clone()
				p1.Mutate(rs, len(goal.Calls), ct, nil, nil, 0, false, false, &Hmdfs_config{}, 0)
				data1 := p1.Serialize()
				if bytes.Equal(want, data1) {
					if !valid {
...
func buildTestContext(test [2]string, target *Target) (rs rand.Source, ct *ChoiceTable, p, goal *Prog, err error) {
	ps, err := target.Deserialize([][]byte{[]byte(test[0])}, Strict)
	if err != nil {
		return
	}
	p = ps[0]
	ps, err = target.Deserialize([][]byte{[]byte(test[1])}, Strict)
	if err != nil {
		return
	}
	goal = ps[0]
