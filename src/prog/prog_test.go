...
func TestGeneration(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		_, _ = target.Generate(rs, 20, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
	}
}
...
func TestDefaultCallArgs(t *testing.T) {
	testEachTarget(t, func(t *testing.T, target *Target) {
		for _, meta := range target.SyscallMap {
			...
			ps, err := target.Deserialize([][]byte{[]byte(prog)}, NonStrict)
			if err != nil {
...
func testSerialize(t *testing.T, verbose bool) {
	...
	for i := 0; i < iters; i++ {
		...
		ps1, err := target.Deserialize([][]byte{data}, mode)
		if err != nil {
...
func testCrossTarget(t *testing.T, target *Target, crossTargets []*Target) {
	...
	for i := 0; i < iters; i++ {
		p, _ := target.Generate(rs, 20, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
		testCrossArchProg(t, p, crossTargets)
		ps, err := target.Deserialize([][]byte{p.Serialize()}, NonStrict)
		if err != nil {
			t.Fatal(err)
		}
		p = ps[0]
		testCrossArchProg(t, p, crossTargets)
		p.Mutate(rs, 20, ct, nil, nil, 0, false, false, &Hmdfs_config{}, 0)
		testCrossArchProg(t, p, crossTargets)
		minPs, _ := Minimize([]*Prog{p}, -1, 0, false, 0, func(ps []*Prog, _ int) bool {
			return rs.Int63()%2 == 0
		})
		p = minPs[0]
		testCrossArchProg(t, p, crossTargets)
...
func testCrossArchProg(t *testing.T, p *Prog, crossTargets []*Target) {
	...
	for _, crossTarget := range crossTargets {
		_, err := crossTarget.Deserialize([][]byte{serialized}, NonStrict)
