...
func TestParseMulti(t *testing.T) {
	...
	if s := entries[0].Ps[0].String(); s != "getpid-gettid" {
		t.Fatalf("bad program 0: %s", s)
	}
	if s := entries[1].Ps[0].String(); s != "getpid-gettid-munlockall" {
		t.Fatalf("bad program 0: %s", s)
	}
	if s := entries[2].Ps[0].String(); s != "getpid-gettid" {
		t.Fatalf("bad program 1: %s", s)
	}
	if s := entries[3].Ps[0].String(); s != "gettid-getpid" {
		t.Fatalf("bad program 2: %s", s)
	}
	if s := entries[4].Ps[0].String(); s != "munlockall" {
...
`
	...
	ent := entries[0]
	faultCall := ent.Ps[0].Calls[1]
	normalCall := ent.Ps[0].Calls[0]
