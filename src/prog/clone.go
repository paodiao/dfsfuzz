...
func (p *Prog) Clone() *Prog {
	p1 := &Prog{
		...
		//IsForSrv: p.IsForSrv,
		SyncIdx:  p.SyncIdx,
		HasCrashFail: p.HasCrashFail,
		HasNetFail: p.HasNetFail,
		IsStash:  p.IsStash,
		IsDCache: p.IsDCache,
		IsFileOps: p.IsFileOps,
		IsInodeOps: p.IsInodeOps,
		BarrierIdx: p.BarrierIdx,
		C2test:  p.C2test,
	}

	//p1.SrvFailPos = [][]int
	for _, item := range p.SrvFailPos {
		p1.SrvFailPos = append(p1.SrvFailPos, append([]int(nil), item...))
	}
	p1.SrvFailOrder = append([]int(nil), p.SrvFailOrder...)
	for _, item := range p.GeneralFailPos {
		p1.GeneralFailPos = append(p1.GeneralFailPos, append([]int(nil), item...))
	}
	for _, item := range p.GeneralBarrierPos {
		p1.GeneralBarrierPos = append(p1.GeneralBarrierPos, append([]int(nil), item...))
	}
	...
	for ci, c := range p.Calls {
		c1 := new(Call)
		c1.Meta = c.Meta
		c1.IsFCall = c.IsFCall
		//c1.Conns = append([]Conn(nil), c.Conns...)
		if c.Ret != nil {
