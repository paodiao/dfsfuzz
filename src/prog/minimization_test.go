...
func TestMinimize(t *testing.T) {
	...
	for ti, test := range tests {
		...
		ps, err := target.Deserialize([][]byte{[]byte(test.orig)}, Strict)
		if err != nil {
			t.Fatalf("failed to deserialize original program #%v: %v", ti, err)
		}
		p := ps[0]
		minPs, ci := Minimize([]*Prog{p}, test.callIndex, 0, false, 0, func(minPs []*Prog, callIndex int) bool {
			return test.pred(minPs[0], callIndex)
		})
		p1 := minPs[0]
		res := p1.Serialize()
...
func TestMinimizeRandom(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		for _, crash := range []bool{false, true} {
			p, _ := target.Generate(rs, 5, ct, nil, false, nil, false, &Hmdfs_config{}, 0)
			copyP := p.Clone()
			minPs, _ := Minimize([]*Prog{p}, len(p.Calls)-1, 0, crash, 0, func(minPs []*Prog, callIndex int) bool {
				p1 := minPs[0]
				if r.Intn(2) == 0 {
					return false
				}
...
func TestMinimizeCallIndex(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		...
		ps1, ci1 := Minimize([]*Prog{p}, ci, 0, r.Intn(2) == 0, 0, func(ps []*Prog, callIndex int) bool {
			return r.Intn(2) == 0
		})
		p1 := ps1[0]
